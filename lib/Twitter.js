...
module.exports = Entity.AbstractComponent.extend({
   ...
   */
  start: function (done) {
    this.log.info(this.toString(), this.name + ' start');
   ...
   */
  stop: function (done) {
    this.log.info(this.toString(), this.name + ' stop');
    done();
  },
  ...
  in_tweet: function (msg) {
    if (this.client) {
      this.client.post('statuses/update', { status: msg }, function (err, res) {
        if (err) {
          if (res.request) {
            this.log.error(this.toString(), this.name + ' unable to tweet ('+err.message+', request='+res.request+', message='+res.error+')');
          } else {
            res.errors.forEach(function (error) {
              this.log.error(this.toString(), this.name + ' unable to tweet (code='+error.code+', message='+error.message+')');
            }.bind(this));
          }
        } else {
          this.log.debug(this.toString(), this.name + ' just tweeted: '+msg);
        }
      }.bind(this));
    }
  }
